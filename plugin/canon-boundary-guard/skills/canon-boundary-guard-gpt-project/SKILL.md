...
Apply the complete protocol from the start of the conversation, before the
first substantive output. Do not wait for an explicit invocation or a
persistence request to activate it. Its existing conditions determine when
each gate, label, proof, state, and recovery rule applies.

Before the first substantive output, inspect the adjacent
`PROJECT_CUSTOM_INSTRUCTIONS.txt` and perform the Required bootstrap below.
The original instructions and references remain binding in full.

For this native plugin distribution, the logical bundle name
`canon-boundary-guard-gpt/` in the preserved documents resolves to the installed
directory containing this `SKILL.md`. Resolve its relative references, schemas,
and scripts from that directory using the actual path or resource identifier
provided by the host. Inspect those resources; do not invent locations,
source identities, hashes, or successful reads. The installed bundle replaces
the uploaded source zip as the distribution source. If an uploaded zip is
actually used, retain the original source-staged extraction and anchoring rules.

In this distribution, the original session-start requirement also applies to
ordinary ChatGPT conversations outside Projects. Project-specific surfaces
still refer to actual Projects. The `/mnt/data/scratch/**`, `/mnt/data/canon/**`,
and `/mnt/data/_SESSION_STATE.json` paths retain their original runtime roles;
they do not resolve into the installed plugin directory.

Installation or availability of this plugin alone does not declare a fresh
state or authorize a reset. Apply the original first-install and recovery
conditions unchanged, including the read-only fallback when required sources
or valid recovery state are unavailable.

<!-- End native plugin binding; original skill body follows unchanged. -->
